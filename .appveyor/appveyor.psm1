function Start-AppveyorInstallTask
{
    ...
    $testHelperPath = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER `
                                -ChildPath "Modules\xNetworking\DscResource.Tests\TestHelper.psm1"
...
function Start-AppveyorAfterTestTask
{
    ...
    $mainModulePath = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER -ChildPath "modules\xNetworking"

    # Write the PowerShell help files
    $docoPath = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER `
                          -ChildPath "modules\xNetworking\en-US"
    ...
    $manifest = Join-Path -Path $env:APPVEYOR_BUILD_FOLDER -ChildPath "modules\xNetworking\xNetworking.psd1"
    (Get-Content $manifest -Raw).Replace("5.2.0.0", $env:APPVEYOR_BUILD_VERSION) | Out-File $manifest
    ...
    $nuspecParams = @{
        ...
        packageDescription = "xNetworking"
